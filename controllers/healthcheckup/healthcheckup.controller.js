...
exports.create = (req, res) => {
    ...
    for (let index = 0; index < body.length; index++) {
        ...
        HealthCheckUpModel.findOne({
            where: { policy_no: policy_no },
        }).then((member) => {

            if (!member) {
                HealthCheckUpModel.create({
                    policy_no,
                    member_name,
                    mobile_number,
                    sms_status,
...
exports.sendSMS = async (req, res) => {
    ...
    const healthCheckup_members = await HealthCheckUpModel.findAll({ where: { sms_status: 0 }, limit: 1, raw: true }).then(deliveries => { return deliveries; }).catch(error => { console.log(error) });

    // var hostname = "http://" + req.headers.host.split(":")[0];
    // hostname = hostname + "/pdms/backend";

    let v4 = "Aditya Birla Health Insurance plan";

    let customUrl = "https://bit.ly/3ml0PYj"
    for (let index = 0; index < healthCheckup_members.length; index++) {
        const healthCheckup_member = healthCheckup_members[index];

        // const doc = await DocumentModel.findOne({ where: { policy_no: delivery.policy_no } }).then(doc => { return doc; }).catch(error => { error });
         
        if (healthCheckup_member.mobile_number != '') {
            // const msg = `Dear ${healthCheckup_member.member_name}, click ${customUrl} to download steps to avail Annual Health Check up for Certificate of Insurance no. ${healthCheckup_member.policy_no} for cover under Aditya Birla Health Insurance plan - DPMPRT`;
            const msg = `Dear ${healthCheckup_member.member_name}, click ${customUrl} to download steps to avail Annual Health Check up for Certificate of Insurance no. ${healthCheckup_member.policy_no} for cover under ${v4} DPMPRT`;

            // const mhg = "http://login.afrapro.in/api/mt/SendSMS?user=dineshmehta1508@gmail.com&password=12345678&senderid=DPMPRT&channel=Trans&DCS=0&flashsms=0&number=919096016308&text=${msg}&route=15&DLTTemplateId=1207163031484175403&PEID=1201159724306525673";
            request(
                `http://198.24.149.4/API/pushsms.aspx?loginID=DPMPrinters&password=Chea@1964&mobile=${healthCheckup_member.mobile_number}&text=${msg}&senderid=DPMPRT&route_id=2&Unicode=0&Template_id=1207167455357730616`,
                function (error, response, body) {
                    console.log(body);
                    if (!error && response.statusCode === 200) {
                        HealthCheckUpModel.update({
                            sms_status: 1,
                            sms_sid: JSON.parse(body).Transaction_ID
                        }, {
                            where: { policy_no: healthCheckup_member.policy_no }
                        }).then(updated => {
                        }).catch(error => {
                            console.log(error);
                        });
                    } else {
                        healthCheckup_members.update({
                            sms_status: 2,
                        }, {
                            where: { policy_no: healthCheckup_member.policy_no }
                        }).then(updated => {
